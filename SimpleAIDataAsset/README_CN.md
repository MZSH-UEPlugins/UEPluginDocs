...
### 模块迁移后 DataAsset 不自动导出

**现象**：将 UDataAsset 子类从模块 A 迁移到模块 B（通过 CoreRedirects），重启编辑器后该类型的 DA 没有自动导出 JSON。

**原因**：Asset Registry 使用缓存文件（`Intermediate/CachedAssetRegistry*.bin`）加速启动。模块迁移后 .uasset 文件本身未变动，缓存中仍存储旧的类路径，导致初始同步时无法将该资产识别为 DataAsset 子类。

**解决方法**：删除项目 `Intermediate/` 目录下的 Asset Registry 缓存文件，然后重启编辑器：

```
del Intermediate\CachedAssetRegistry*.bin
```

重启后 Asset Registry 会全量扫描所有资产，CoreRedirects 生效，插件即可正常发现和导出。
