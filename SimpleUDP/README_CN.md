...
> **重要：** `Max Receive Bytes` 默认值为 `1024` 字节。如果协议中的单个 UDP 数据包可能超过 1024 字节，请务必在接收前调大对应接收通道的该参数，否则数据包可能无法被完整接收。
...
> **重要：** `Max Send Bytes` 默认值为 `1024` 字节。如果协议中的单个 UDP 数据包可能超过 1024 字节，请务必在发送前调大对应发送通道的该参数，否则只会发送配置上限以内的数据。
